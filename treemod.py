...
class Node:
    def __init__(self, leftTime="", rightTime="", isdir=None, next=None, child=None, name=None, status=None, path=None, parent=None):
        self.next = next
        self.child = child
        self.status = status #diffした時の結果．l,r,d,iのいずれか / should be one from l,r,d,i
        self.name = name
        self.path = path
        self.parent = parent
...
def make_Tree(tree, path, status):
    var = 0
    key = ["/".join(path[0:i+1]) for i in range(len(path))] #挿入するノードのキー=そのノードのrootからのフルパス / key of node inserted = full path from root of the node
    while var < len(path):
        left_path = tree.left + "/" + key[var] #full path 
        right_path = tree.right + "/" + key[var] #full path
